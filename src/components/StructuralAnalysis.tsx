...
export function StructuralAnalysis() {
    ...
    const renderLoadGraphic = (load: Load) => {
        ...
        if (load.type === 'udl') {
            ...
            return <g key={`load-${load.id}`} {...gP}>{arrs}<line x1={x1-dir*ox*20/zoom} y1={y1-dir*oy*20/zoom} x2={x2-dir*ox*20/zoom} y2={y2-dir*oy*20/zoom} stroke={lc} strokeWidth={sw} /><text x={(x1+x2)/2} y={(y1+y2)/2-dir*ox*25/zoom} {...txS} fill={lc} className="font-black">{Math.abs(load.magnitude/1000).toFixed(1)}kN/m</text></g>;
        }
        return null;
    };
        ...
        <section className="bg-slate-900 border border-slate-800 rounded-[3rem] p-10 shadow-2xl space-y-10">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                <div className="lg:col-span-3 space-y-8 max-h-[600px] overflow-y-auto custom-scrollbar pr-2">
                    ...
                    <Button onClick={handleAnalyze} className="w-full bg-sky-500 text-slate-950 font-black py-6 rounded-2xl transition-all uppercase tracking-widest text-[10px] shadow-xl hover:bg-sky-400 active:scale-95">Run Analysis</Button>
                </div>
